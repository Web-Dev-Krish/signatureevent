...
export function SectionHeader({ eyebrow, title, text }: { eyebrow: string; title: string; text?: string }) {
  return (
    <Reveal className="mx-auto mb-12 max-w-3xl text-center">
      ...
      <h2 className="font-display text-4xl leading-tight text-slate-900 md:text-6xl">{title}</h2>
      {text && <p className="mt-5 text-base leading-8 text-slate-900/65 md:text-lg">{text}</p>}
    </Reveal>
  );
}
...
export function GoldButton({ children, className = '', variant = 'solid' }: { children: ReactNode; className?: string; variant?: 'solid' | 'outline' }) {
  const styles = variant === 'solid'
    ? 'bg-gradient-to-r from-gold to-brightGold text-white shadow-gold'
    : 'border border-gold/70 bg-slate-50 text-slate-900 hover:bg-gold/10';
...
export function GlassCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`rounded-[2rem] border border-slate-200 bg-white/[0.045] shadow-2xl shadow-black/30 backdrop-blur-xl ${className}`}>{children}</div>;
}
...
export function StatCounter({ value, suffix = '', label }: { value: number; suffix?: string; label: string }) {
  ...
  return (
    <div ref={ref} className="rounded-3xl border border-gold/20 bg-black/40 p-6 text-center">
      ...
      <p className="mt-2 text-sm uppercase tracking-[0.18em] text-slate-900/55">{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      {items.map((item, index) => (
        <div key={item.q} className="overflow-hidden rounded-2xl border border-slate-200 bg-white/[0.04]">
          <button onClick={() => setOpen(open === index ? -1 : index)} className="flex w-full items-center justify-between gap-4 p-5 text-left text-slate-900">
            <span className="font-semibold">{item.q}</span>
            <ChevronDown className={`h-5 w-5 text-gold transition ${open === index ? 'rotate-180' : ''}`} />
          </button>
          {open === index && <div className="px-5 pb-5 leading-7 text-slate-900/65">{item.a}</div>}
        </div>
      ))}
    </div>
  );
