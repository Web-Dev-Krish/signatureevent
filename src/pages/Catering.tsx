...
export default function Catering() {
  ...
  return (
    <div className="bg-[#050505] min-h-screen">
      ...
      <div className="relative h-[60vh] flex items-center justify-center text-center overflow-hidden">
        ...
        <div className="relative z-10 px-4">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto font-light">Exquisite culinary experiences tailored for your grand celebrations.</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="py-20 relative z-10 border-b border-white/5">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-serif font-bold text-white mb-4">Premium <span className="text-[#D4AF37]">Packages</span></h2>
            <p className="text-gray-400 max-w-2xl mx-auto">Choose from our meticulously crafted catering packages, designed to delight every palate.</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {packages.length === 0 && !loading && (
              <p className="text-center col-span-3 text-gray-500">Packages will be available soon.</p>
            )}
            {packages.map((pkg, idx) => (
              <motion.div 
                ...
                className={`relative rounded-2xl overflow-hidden backdrop-blur-md border ${
                  idx === 1 ? 'bg-gradient-to-b from-[#1A1500] to-[#0B0B0B] border-[#D4AF37]/50 shadow-[0_0_30px_rgba(212,175,55,0.15)] transform md:-translate-y-4' : 'bg-white/5 border-white/10 hover:border-white/20'
              ...
              >
                ...
                <div className="p-8 flex-1 flex flex-col">
                  ...
                  {pkg.description && <p className="text-gray-400 text-sm mb-6 pb-6 border-b border-white/10">{pkg.description}</p>}
                  ...
                  <a href="tel:+919354710637" className={`w-full block text-center py-4 rounded-xl font-bold uppercase tracking-wider transition-all duration-300 ${
                    idx === 1 
                      ? 'bg-gradient-to-r from-[#D4AF37] to-[#F3E5AB] text-black shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:shadow-[0_0_30px_rgba(212,175,55,0.5)]' 
            ...
            {filteredItems.map((item, idx) => (
              ...
              >
                ...
                <div className="p-5 text-center relative z-10 transform -translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                  <h4 className="text-lg font-bold text-white mb-1 tracking-wide">{item.name}</h4>
      ...
      {/* Live Counters */}
      <div className="py-20 relative border-t border-white/5 bg-gradient-to-b from-transparent to-[#0B0B0B]">
          ...
          <div className="grid md:grid-cols-3 gap-6 text-left">
            {[
              ...
              <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-2xl flex items-start gap-4">
                ...
                <span className="text-gray-300 font-medium">{counter}</span>
              </div>
            ))}
          </div>
        </div>
