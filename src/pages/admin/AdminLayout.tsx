...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-[#FAFAFA] text-slate-600 font-sans">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 w-64 bg-[#FFFFFF] border-r border-slate-200 flex flex-col z-50 transform transition-transform duration-300 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          <Link to="/admin" className="text-xl font-serif font-bold text-slate-900 tracking-wider flex items-center gap-2">
            <span className="text-[#D4AF37]">M</span>ALHOTRA <span className="text-xs text-[#D4AF37] ml-2">CMS</span>
          </Link>
          <button className="lg:hidden text-slate-600 hover:text-slate-900" onClick={() => setIsMobileOpen(false)}>
            <X size={20} />
          </button>
        </div>
          ...
          <nav className="space-y-1 px-3">
            {sidebarLinks.map((link) => {
              ...
              return (
                ...
                >
                  <link.icon size={18} className={isActive ? 'text-[#D4AF37]' : 'text-slate-600'} />
                  {link.name}
                </Link>
              );
            })}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Header */}
        <header className="h-16 bg-[#FFFFFF] border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 shrink-0">
          <button className="lg:hidden text-slate-600 hover:text-slate-900" onClick={() => setIsMobileOpen(true)}>
            <Menu size={24} />
          </button>
