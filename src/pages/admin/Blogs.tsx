  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-serif font-bold text-slate-900">Manage Blogs</h1>
      ...
      <div className="bg-[#FFFFFF] border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-white/5">
            {loading ? <tr><td colSpan={4} className="p-8 text-center text-slate-600">Loading...</td></tr> : blogs.map(b => (
              <tr key={b.id} className="hover:bg-slate-50">
                <td className="p-4 text-slate-900 font-semibold">{b.title}</td>
                <td className="p-4 text-slate-600">{b.category}</td>
                <td className="p-4 text-slate-600">{formatDate(b.published_at)}</td>
                <td className="p-4 text-right flex justify-end gap-2">
                  <button onClick={() => editBlog(b)} className="p-2 bg-slate-50 text-slate-600 rounded hover:text-slate-900"><Edit2 size={16} /></button>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-[#FFFFFF] border border-slate-200 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center">
              <h2 className="text-xl font-bold text-slate-900">{editingId ? 'Edit Post' : 'New Post'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required className="w-full bg-[#FAFAFA] border border-slate-200 rounded p-2 text-slate-900" value={form.category} onChange={e => setForm({...form, category: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm text-slate-600 mb-1">Author</label>
                  <input required className="w-full bg-[#FAFAFA] border border-slate-200 rounded p-2 text-slate-900" value={form.author} onChange={e => setForm({...form, author: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-sm text-slate-600 mb-1">Cover Image URL</label>
              ...
              <div>
                ...
                <textarea required rows={8} className="w-full bg-[#FAFAFA] border border-slate-200 rounded p-2 text-slate-900" value={form.content} onChange={e => setForm({...form, content: e.target.value})} />
              </div>
