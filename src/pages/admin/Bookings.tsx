  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-serif font-bold text-slate-900">Manage Bookings</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr><td colSpan={5} className="p-8 text-center text-slate-600">Loading...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-slate-600">No bookings found</td></tr>
              ) : (
                filtered.map(b => (
                  <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4">
                      ...
                      {b.phone && <div className="text-xs text-slate-600">{b.phone}</div>}
                    </td>
                    <td className="p-4 text-slate-600">{formatDate(b.date)}</td>
                    ...
                    <td className="p-4 text-right flex justify-end gap-2">
                      ...
                      <button onClick={() => deleteBooking(b.id)} className="p-2 bg-gray-500/10 text-slate-600 rounded hover:bg-gray-500/20 hover:text-red-500" title="Delete">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
