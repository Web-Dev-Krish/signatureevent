  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-serif font-bold text-gray-900">Video Assets</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? <p className="text-gray-600">Loading...</p> : videos.map(vid => (
          <div key={vid.id} className="bg-[#FFFFFF] border border-black/5 rounded-xl overflow-hidden group">
            <div className="relative h-48 bg-black">
              {vid.thumbnail_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-[#F9F6F0]">
              ...
              <a href={vid.video_url} target="_blank" rel="noreferrer" className="absolute inset-0 flex items-center justify-center">
                <PlayCircle size={48} className="text-gray-900 drop-shadow-lg group-hover:scale-110 transition-transform" />
              </a>
            </div>
            <div className="p-4 flex justify-between items-center">
              <h3 className="text-gray-900 font-semibold truncate flex-1 pr-4">{vid.title}</h3>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-[#FFFFFF] border border-black/5 rounded-xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">Video URL (YouTube, Vimeo, MP4)</label>
                <input required className="w-full bg-[#F9F6F0] border border-black/5 rounded p-2 text-gray-900" value={form.video_url} onChange={e => setForm({...form, video_url: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm text-gray-600 mb-1">Thumbnail URL</label>
                <input required className="w-full bg-[#F9F6F0] border border-black/5 rounded p-2 text-gray-900" value={form.thumbnail_url} onChange={e => setForm({...form, thumbnail_url: e.target.value})} />
              </div>
